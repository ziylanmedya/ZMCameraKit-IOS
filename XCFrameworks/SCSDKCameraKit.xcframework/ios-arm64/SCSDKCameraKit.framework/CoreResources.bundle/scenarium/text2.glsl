...
#error Unknown fillmode
...
#ifdef BACKGROUND_FILL_COLOR
        float alpha = backgroundColor.a * cornerFade;
        gl_FragColor = vec4(backgroundColor.rgb * alpha, alpha);
#elif defined BACKGROUND_FILL_TEXTURE
        ...
        float alpha = backgroundFillTextureSample.a * cornerFade;
        gl_FragColor = vec4(backgroundFillTextureSample.rgb * alpha, alpha);
#else
#error Unknown fillmode
#endif
        return;
    ...
    if(isPass(COLOR_PASS, passIdentifier)) {
        ...
        float alpha = fillColor.a * sdfOpacity;
        gl_FragColor = vec4(fillColor.rgb * alpha, alpha);
#else
        float sourceAlpha = texture2D(mainTexture, varTex0).r;
        float alpha = sourceAlpha * fillColor.a;
        gl_FragColor = vec4(fillColor.rgb * alpha, alpha);
#endif //ENABLE_SDF
