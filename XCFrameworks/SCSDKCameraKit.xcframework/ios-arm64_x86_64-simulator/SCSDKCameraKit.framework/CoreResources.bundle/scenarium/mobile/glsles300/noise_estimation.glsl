...
//spec_const int SC_SOFTWARE_WRAP_MODE_U_inputTexture 5 -1
//spec_const int SC_SOFTWARE_WRAP_MODE_V_inputTexture 6 -1
//spec_const int inputTextureLayout 7 0
...
{
float l9_0=6.2831855*fract(sin(dot(uv+vec2(seed.y),vec2(12.9898,78.233002)))*43758.547);
return vec2(cos(l9_0),sin(l9_0))*sqrt((-2.0)*log(max(fract(sin(dot(uv+vec2(seed.x),vec2(12.9898,78.233002)))*43758.547),1e-06)));
}
...
#endif
vec4 l9_0=sampleInputTexture(varTex01.xy,0.0);
vec4 l9_1=sampleInputTexture(varTex01.xy,5.0);
vec2 l9_2=fwidth(varTex01.xy);
vec2 l9_3=floor(varTex01.xy/(l9_2*2.0))*l9_2;
vec4 l9_4=vec4(boxMuller(l9_3,seeds.xy),boxMuller(l9_3,seeds.zw))*stddev;
vec3 l9_5=clamp(vec3(srgbToLinear(l9_0.x),srgbToLinear(l9_0.y),srgbToLinear(l9_0.z))+(((l9_4.xyz*rgbRatio)+vec3(l9_4.w*(1.0-rgbRatio)))*sqrt(1.0-max(dot(vec3(srgbToLinear(l9_1.x),srgbToLinear(l9_1.y),srgbToLinear(l9_1.z)),vec3(0.29899999,0.58700001,0.114)),0.0))),vec3(0.0),vec3(1.0));
sc_writeFragData0Internal(vec4(linearToSrgb(l9_5.x),linearToSrgb(l9_5.y),linearToSrgb(l9_5.z),l9_0.w),sc_UniformConstants.x,sc_ShaderCacheConstant);
}
